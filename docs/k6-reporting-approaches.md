...
Covered in [cost-comparison.md](cost-comparison.md) (Optional hosted observability). The key cost split: **Grafana Cloud k6** (`k6 cloud run`) bills **VUh** even with `--local-execution`; **Prometheus remote-write to Grafana Cloud** does not (it counts against the active-series cap). Out of scope here because the whole motivation was avoiding VUh.
