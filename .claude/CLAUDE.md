...
- `-e FIDELITY=<lean|ui|full>` raises a journey's fidelity: `lean` (spine only, default) → `ui` (adds the UI-chrome replay tier) → `full` (adds static assets). The chrome/static tiers are generated from a NeoLoad recording and correlated at runtime — see `rules/fidelity.md`. A journey paces itself between steps with `think()` (uniform-random 2–3s matching NeoLoad's `P_thinkTime`, recorded as the `think_time` metric)
- Load profiles (`load`, `stress`, `neoload`) live in `source/config/profiles.config.ts` (`load_profile()`, selected with `-e PROFILE=`). `source/tests/neoload.spec.ts` is the wired load spec: it spreads `load_profile('neoload')` into one `ramping-vus` scenario per flow (5m ramp / 30m sustain / 5m down @10 VUs each, five flows = 50 VUs peak) and paces each iteration to a fixed cycle via `pace()` (`-e PACING=`, default 300s). It asserts `commonThresholds` + each journey's SLAs (see `rules/tests.md`)
...
- `k6 inspect --execution-requirements source/tests/<name>.spec.ts` — also zero traffic; for a load spec that consumes `load_profile()` (e.g. `neoload.spec.ts`), this resolves the stage spread and computes its max VUs / total duration, so a broken stage config is caught without running load
...
- k6 on Windows does not honor the `TZ` env var (the run stays on the machine's local zone), so a timezone-dependent code path can't be exercised by setting `TZ`; to test one, simulate the offset inside the script (a temporary env-gated shift of the epoch) and remove it after verifying
...
- `k6-tests-ci.yml` — **manually queued** (`trigger: none`), self-hosted pool. Actually runs `k6 run` on `source/tests/neoload.spec.ts` (`neoload` profile), samples agent + k6-runtime resource usage via `.azure/scripts/`, publishes the `k6-results` artifact, then deletes the plaintext secret. The decryption passphrase comes from the masked `K6_DECRYPT_KEY` pipeline variable (never committed).
