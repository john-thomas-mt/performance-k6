---
paths: ["source/scenarios/**"]
---

# Scenario Script Conventions (`source/scenarios/`)

The entry-point layer k6 runs directly. A scenario file drives one or more journeys in a single run via k6 `scenarios` in its `options`. Each scenario's `exec` names a thin wrapper exported from the file that picks a user (`pickUser(users)`) and calls a `<journey>Journey(...)` function from `source/flows/` (see `rules/flows.md`) — journey logic is never duplicated here. Like `source/seeds/`, this is an entry-point folder nothing imports, so it has no barrel.

## File naming
- `<name>.scn.ts`, kebab-case (e.g. `smoke.scn.ts`) — the `.scn.ts` suffix marks a scenario script

## Smoke aggregate — register every journey
- `source/scenarios/smoke.scn.ts` runs every journey once as the correctness/drift gate: one `per-vu-iterations` scenario per journey, each `exec` a thin wrapper calling that journey. Its `setup()` returns the superset of data the journeys need (`{ version, pool }`)
- **Adding a journey means registering it**: add the flow's `export *` line to `source/utils/exports/flows.exp.ts`, then in `smoke.scn.ts` add a scenario entry, its `exec` wrapper, and its `<journey>Thresholds` to the threshold map. A journey not registered there is invisible to the smoke gate and the `validate-payload-drift` skill
- Run one journey at scripting time with `-e SCENARIO=<name>` (the file filters its `scenarios` down to that entry and rejects an unknown name); scale that dev run with `-e VUS=` / `-e ITERS=`. With no `-e SCENARIO`, every journey runs — the full drift gate
...
- `smoke.scn.ts` merges `commonThresholds` + `loginThresholds` + the `<journey>Thresholds` of the journey(s) it runs — all of them for the full gate, just the selected one under `-e SCENARIO`. Gating to the active journeys keeps every asserted threshold pointed at an endpoint the run actually exercises

## Execution shape
- `smoke.scn.ts` fixes its own small `per-vu-iterations` shape, overridable via `-e VUS=`/`-e ITERS=` for the dev ladder
- For real load, shape executors from `loadProfile()` (`source/config/profiles.config.ts`, selected with `-e PROFILE=`) rather than hardcoding `vus`/`stages`
