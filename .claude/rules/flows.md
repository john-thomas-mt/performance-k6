...
- `login.flow.ts` is the shared entry point: `login_to_momentus_assistant(user, version)` owns journey groups 1–2 and returns the session `{ bearerToken, salesAiJwt }`; `login_to_events(user, version)` returns `{ bearerToken, encUserId }`
