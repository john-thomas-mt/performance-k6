...
Render any date or timestamp sent to the server as a string — an optimistic-concurrency stamp, a formatted date cell — in **UTC** (`getUTC*`, `Date.UTC`), never local-time getters (`getHours`/`getFullYear`/…). A local-time render makes the string depend on the runner's machine timezone: the same code passes on a dev box in one zone and fails on a CI agent in another — an epoch rendered in local time and parsed by the server as UTC lands hours off, tripping a stale-stamp concurrency rejection. Mirror the UTC-correct date helper already in the repo rather than re-deriving the format.
