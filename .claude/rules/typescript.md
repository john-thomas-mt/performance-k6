...
## Naming (org convention, enforced by `.config/eslint/eslint.config.mjs`)
- Functions and class methods are `snake_case`; types/interfaces/enums are `PascalCase`; variables and parameters are `camelCase`, `UPPER_CASE`, or `snake_case`. This is the org typescript-eslint standard — author new identifiers to it rather than the camelCase TS default; the eslint config is the enforced source of truth. Request-tag names and check labels are string literals, not identifiers, so they stay `PascalCase` (`'GetOpportunities'`, `'SignIn: status is 201'`).
- k6 lifecycle exports keep the names k6 requires: `setup`/`teardown` are already `snake_case`-valid; a `default` export and `handleSummary` are exempt — disable the rule inline if `handleSummary` is added.
