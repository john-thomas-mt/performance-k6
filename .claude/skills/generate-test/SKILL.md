...
- **Fidelity is a separate lever.** A live-authored journey ships at `lean` (the spine you scripted). The `-e FIDELITY=ui|full` chrome/static replay tiers are sourced from a **NeoLoad recording** (the `neoload-to-k6` skill), not from live exploration — don't build them here; see `rules/fidelity.md`.
