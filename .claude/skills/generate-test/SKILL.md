...
- New endpoints get a thin wrapper in `source/apis/<feature>.api.ts`.
- Decide the data-setup strategy before scripting the journey: provision a unique data hierarchy rather than depending on a hand-provided shared record. `setup()` seeds a run-scoped parent, each iteration creates its own mutation target, and `teardown()` cascade-deletes the parent — tagging provisioning requests separately from the operation under test. (The lifecycle/tagging/cleanup specifics auto-load from the tests rules when you edit the spec.)
- Pick the VU's user with `pickUser(users)` from `source/helpers/users.helper.ts` (honors `USER_MODE`), not an inline `users[__VU % users.length]`.
