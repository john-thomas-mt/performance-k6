...
export function editServiceOrdersJourney(user: User, data: ServiceOrderSetup, documentFile: ArrayBuffer) {
  ...
  group('3. Open Service Order', () => {
    ...
    stamps = readOrderHeaderStamps(res);
  });
  ...
  group('4. Edit General (rate & order date)', () => {
    editServiceOrderGeneral(bearerToken, data.version, serviceOrder, orderDate, orderStamps);
  });

  group('5. Add & Save Service Order Items', () => {
    saveServiceOrderItems(bearerToken, data.version, serviceOrder, ITEM_QUANTITY);
  });
  ...
  group('6. Upload & Import Document', () => {
    ...
    saveDocument(bearerToken, data.version, serviceOrder, doc);
  });
  ...
  group('7. Save & Close', () => {
    ...
    const res = openServiceOrderDetail(bearerToken, data.version, serviceOrder);
    const fresh = readOrderHeaderStamps(res);
    saveAndCloseServiceOrder(bearerToken, data.version, serviceOrder, orderDate, fresh);
  });
