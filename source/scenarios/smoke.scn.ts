...
import { ServiceOrderPoolSetup } from '../utils/exports/types.exp.ts';
...
export async function setup(): Promise<ServiceOrderPoolSetup> {
  ...
  const pool = discoverServiceOrderPool(version, users[0]);
  console.log(`Server version: ${version}`);
  console.log(`Smoke: ${pool.length} seeded service order(s) discovered`);
  return { version, pool, users };
}

export function opportunities(data: ServiceOrderPoolSetup) {
  opportunitiesJourney(pickUser(data.users), data);
}

export function fileUpload(data: ServiceOrderPoolSetup) {
  fileUploadJourney(pickUser(data.users), data, opportunityTemplate);
}

export function introductoryEmail(data: ServiceOrderPoolSetup) {
  introductoryEmailJourney(pickUser(data.users), data);
}

export function copyEvent(data: ServiceOrderPoolSetup) {
  copyEventJourney(pickUser(data.users), data);
}

export function serviceOrderItems(data: ServiceOrderPoolSetup) {
